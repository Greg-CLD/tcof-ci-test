 ...
 * 
 * Usage:
 *   node scripts/assert-checklist.js <PROJECT_UUID> <EXPECTED_COUNT>
 * 
 * Example:
 *   node scripts/assert-checklist.js bc55c1a2-0cdf-4108-aa9e-44b44baea3b8 4
 * 
 * Description:
...
if (!projectId || !validateUuid(projectId)) {
  ...
  console.error('Usage: node scripts/assert-checklist.js <PROJECT_UUID> <EXPECTED_COUNT>');
  process.exit(1);
}
...
async function runTest() {
  ...
  try {
    ...
    const invalidUuidTasks = updatedTasks.filter(task => 
      !validateUuid(task.id) || 
      !validateUuid(task.projectId)
    );
