...
async function processExcelAndUpdateDB() {
  try {
    ...
    // Now that we can see the column names, let's adjust our field names and process the data
    const tasks = [];
    
    // Process data with correct field names (after we see the Excel structure)
    for (const row of excelData) {
      // We will finalize these field names after seeing the Excel data
      const factorIdField = 'FactorID'; // Placeholder field name
      const stageField = 'Stage';       // Placeholder field name
      const taskField = 'Task';         // Placeholder field name
      
      const factorId = row[factorIdField];
      const stage = row[stageField];
      const task = row[taskField];
      
      if (factorId && stage && task) {
