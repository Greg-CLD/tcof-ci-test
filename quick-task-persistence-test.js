...
function captureSessionId() {
  ...
  // Run a command to extract session info from the logs
  exec("grep -A 5 'Session debug' ./.replit/logs/console.log | grep 'sessionID' | tail -n 1 > ./current-session.txt", (error) => {
    if (error) {
      console.error(`${YELLOW}Could not extract session ID from logs: ${error.message}${RESET}`);
      console.log(`${YELLOW}Will use fallback session ID.${RESET}`);
    } else {
      console.log(`${GREEN}Session ID captured from logs.${RESET}`);
    }
