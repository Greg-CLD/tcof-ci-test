...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/user', isAuthenticated, async (req: any, res) => {
    try {
      // Get user ID directly from req.user (local auth)
      ...
      // Return the user object directly, already populated by passport
  ...
  try {
    ...
    app.use('/api/projects', tasksRoutes.default);
  ...
  async function saveFactors(factors: FactorTask[]): Promise<boolean> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to save factors';
      console.error(errorMessage);
      return false;
    }
  }
  ...
  app.get('/api/admin/tcof-tasks', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to load core tasks data';
  ...
  app.post('/api/admin/tcof-tasks', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to save success factors data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.get('/api/admin/success-factors', isAdmin, async (req: Request, res: Response) => {
    try {
      console.log('Admin API: Getting all success factors with detailed debugging...');
      
      // Let's try a step-by-step approach to identify exactly where the issue is happening
      
      // First, let's directly get the SQL output and thoroughly examine it
      const rawQuery = `
        SELECT 
          id, 
          title, 
          description, 
          tasks::text as tasks_json 
        FROM 
          v_success_factors_full 
        WHERE 
          id = 'sf-1'
        ORDER BY 
          title;
      ...
      console.log('Executing raw SQL query against the view for a single factor');
      const rawResult = await db.execute(rawQuery);
      
      if (rawResult.rows.length > 0) {
        console.log('RAW SQL OUTPUT FOR SF-1:');
        console.log(JSON.stringify(rawResult.rows[0], null, 2));
        
        // Check the JSON structure explicitly
        const rawTasksJson = rawResult.rows[0].tasks_json;
        console.log('TASKS JSON STRING (direct from DB):');
        console.log(rawTasksJson);
        ...
        try {
          ...
          // Check if the identification tasks exist and are not null
          if (parsed.Identification) {
            console.log('IDENTIFICATION TASKS ARRAY:');
            console.log(JSON.stringify(parsed.Identification, null, 2));
            
            // Check each task
            if (Array.isArray(parsed.Identification)) {
              parsed.Identification.forEach((task: any, index: number) => {
                console.log(`Task ${index}: ${task === null ? 'NULL' : `"${task}"`}`);
              });
              
              // Filter out nulls to see what we get
              const filtered = parsed.Identification.filter(Boolean);
              console.log(`After filtering nulls: ${filtered.length} tasks remain:`);
              filtered.forEach((task: string, index: number) => {
      ...
      // Now let's use a completely different approach for the list endpoint
      // Instead of using the view, let's directly query tasks table and build the structure manually
      console.log('Fetching factors and tasks directly from tables...');
      
      // First get all factors
      const factorsQuery = `SELECT id, title, description FROM success_factors ORDER BY title`;
      ...
      tasksResult.rows.forEach((task: any) => {
        ...
        // Add task to the appropriate stage array
        if (tasksByFactor[factorId][stage] && text) {
          tasksByFactor[factorId][stage].push(text);
        }
      });
      
      // Now build the final factors list
      const formattedFactors = factorsResult.rows.map((factor: any) => {
        const factorId = factor.id;
        
        // Get tasks for this factor or use empty structure
        const factorTasks = tasksByFactor[factorId] || {
          'Identification': [],
          'Definition': [],
          'Delivery': [],
          'Closure': []
      ...
      // Log the first factor to verify
      if (formattedFactors.length > 0) {
        const sample = formattedFactors[0];
        console.log(`First factor ${sample.id} has tasks:`, JSON.stringify(sample.tasks, null, 2));
      }
      
      console.log(`Returning ${formattedFactors.length} factors with manually constructed tasks`);
  ...
  app.post('/api/admin/deduplicate-factors', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to deduplicate factors';
      res.status(500).json({ message: 'Failed to deduplicate factors', error: errorMessage });
    }
  });
  ...
  app.get('/api/admin/graph', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate graph data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.get('/api/admin/graph/neighbours/:nodeId', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate neighbour graph data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.get('/api/admin/relations-export', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to export relations data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.get('/api/admin/factors-integrity', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to generate factors integrity report';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.post('/api/admin/update-canonical-factors', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update canonical factors';
      res.status(500).json({ message: 'Failed to update canonical factors', error: errorMessage });
    }
  });
  ...
  app.get('/api/admin/preset-heuristics', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to load preset heuristics data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.post('/api/admin/update-factor-tasks', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error updating factor tasks: ';
      res.status(500).json({ error: errorMessage + (error instanceof Error ? error.message : "") });
    }
  });
  ...
  app.post('/api/admin/preset-heuristics', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to save preset heuristics data';
      res.status(500).json({ message: errorMessage });
    }
  });
  ...
  app.get('/api/admin/factors-export', isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to export success factors data';
      res.status(500).json({ message: errorMessage });
    }
  });
