...
router.get('/plans/project/:projectId/block/:blockId', async (req, res) => {
  try {
    ...
    if (block) {
      ...
      console.info(`[LOAD-API] personalHeuristics: ${block.personalHeuristics ? 'Present' : 'MISSING'}`);

      return res.status(200).json({
        id: block.id,
        successCriteria: block.successCriteria || [],
        successFactorRatings: block.successFactorRatings || {},
        personalHeuristics: block.personalHeuristics || []
      });
    }
