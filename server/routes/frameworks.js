...
export async function saveProject(projectId, frameworksData) {
  try {
    const filePath = path.join(FRAMEWORKS_DIR, `${projectId}.json`);
...
export async function getProjectFrameworks(req, res) {
  try {
...
export async function saveProjectFrameworks(req, res) {
  try {
    const { projectId } = req.params;
