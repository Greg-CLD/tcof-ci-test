 ...
 * 
 * Using .cjs extension to run in CommonJS mode
 */

const { Client } = require('pg');

// Function to extract UUID from potentially compound IDs - same as in our client code
...
async function testUuidHandling() {
  ...
  try {
    ...
    // Step 5: Reset the task to its original state for clean testing
    console.log('\nResetting task to original state...');
    await client.query(`
      UPDATE project_tasks
      SET completed = $1, updated_at = NOW()
      WHERE id = $2
    `, [originalTask.completed, originalTask.id]);
